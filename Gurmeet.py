...
BAWA_ok = f'OK-{hari}-{bulan}-{tahun}.txt'
BAWA_cp = f'CP-{hari}-{bulan}-{tahun}.txt'
...
def cek_akun():
	...
	xxx = input(' BAWApan akun tidak kenon ke file apa : \n nama : ')
	nonon = xxx+'.txt'
	file = nom[int(exc)-1]
	print(' ─────────────────────────────')
				...
				if data in rescp:pass
					...
					if 'ya' in cepeh:
						...
						try:
							...
							sapi = f'{idf}|{pw}|{d} {m} {y}'
							open('CP/'+BAWA_cp,'a').write(sapi+'\n')
							break
						except:
							print(f'\n [{kk}>{P}] email  : {kk}{idf}{P}          \n [{kk}>{P}] sandi  : {kk}{pw}          {P}\n')
							os.popen('play-audio data/cepe.mp3')
							open('CP/'+BAWA_cp,'a').write(idf+'|'+pw+'\n')
							break
...
def cek_opsi(idf,pw,ua):
	...
	try:
		...
		else:open('CP/'+BAWA_cp,'a').write(mek+'\n')
	except:
		month = ""
		day = ""
		year = ""
		akun += f' [{kk}>{P}] email  : {kk}{idf}{P}          \n [{kk}>{P}] sandi  : {kk}{pw}          {P}'
		if 'file' in detek:pass
		else:open('CP/'+BAWA_cp,'a').write(idf+'|'+pw+'\n')
	try:
